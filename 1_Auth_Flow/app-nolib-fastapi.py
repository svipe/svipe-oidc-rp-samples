...
def oidc_get_userinfo(access_token):
    ...
    if resp.status_code == 200:
        print("got userinfo: ", resp.json())
        return resp.json()

def oidc_get_login_redirect(state, nonce, code_verifier, redirect_uri):
    params = {
    ...
    if code_verifier:
        params['code_challenge'] = oidc_create_code_challenge(code_verifier)

    return urllib.parse.urljoin(
        openid_config.get('authorization_endpoint'), 
